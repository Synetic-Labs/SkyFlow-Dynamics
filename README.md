...
- **Authenticity** (`golden/`) — frozen `(state, input, params) → derivative/next-state`
  ...
  Crazyflow, SkyDreamer), stored with commit-level provenance.
...
```bash
# RotorPy (run from an upstream spencerfolk/rotorpy checkout):
cd ../rotorpy && uv run --no-project --with numpy --with scipy python \
